...
/// Get making amount from FillResult
public fun get_making_amount(result: &FillResult): u256 {
    result.making_amount
}

/// Get taking amount from FillResult
public fun get_taking_amount(result: &FillResult): u256 {
    result.taking_amount
}

/// Get order hash from FillResult
public fun get_order_hash(result: &FillResult): vector<u8> {
    result.order_hash
}
...
/// Fill an order (following Ethereum fillOrder logic)
/// This function validates the order, performs token transfers, and emits events
/// This is the main function that does everything like Ethereum's fillOrder
public fun fill_order<T, U>(
    ...
    // Follow Ethereum _fillOrder logic - validate order
    let (order_hash, maker, making_amount, taking_amount) = validate_and_calculate_fill(
        order_book, order, signature, amount, taker_traits, clock, ctx
    );
    
    // Validate that amounts fit in u64 for Sui coins
    ...
    // Perform actual token transfers (like Ethereum fillOrder)
    execute_coin_swap(
        maker_coin,
        taker_coin,
    ...
    // Emit event (following Ethereum OrderFilled event)
...
/// Validate order and calculate fill amounts
fun validate_and_calculate_fill(
    order_book: &mut OrderBook,
    order: &Order,
    signature: &SuiSignature,
    amount: u256,
    ...
    clock: &Clock,
    ctx: &mut TxContext
): (order_types::OrderHash, FusionAddress, u256, u256) {
    let order_hash = sui_signing::hash_order_blake2b(
        order_types::salt(order),
        order_types::maker(order),
        order_types::receiver(order),
    ...
    (order_hash, maker, making_amount, taking_amount)
}
...
/// Execute coin swap with actual transfers
public fun execute_coin_swap<T, U>(
    ...
    // 1. Pre-interaction (maker prepares funds) - skipped for now
    // 2. Maker → Taker transfer
    // 3. Taker interaction (optional) - skipped for now
    // 4. Taker → Maker transfer
    // 5. Post-interaction (maker handles funds) - skipped for now
    ...
    // Step 3: Taker interaction (optional) - skipped for now
    // In Ethereum: ITakerInteraction(address(bytes20(interaction))).takerInteraction(...)
    // TODO: Implement taker interaction when taker interaction contracts are available
    ...
    // Step 5: Post-interaction (maker handles funds) - skipped for now
    // In Ethereum: if (order.makerTraits.needPostInteractionCall())
    // TODO: Implement post-interaction when maker interaction contracts are available
    
    // Step 6: Emit final swap event (following Ethereum OrderFilled event)
